...
class ControllerApiOrder extends Controller {
	...
	public function info() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			$order_info = $this->model_checkout_order->getAPIOrder($order_id);
	...
	public function all(){
		...
		if (!isset($this->session->data['api_id'])) {
			...
			$order_info = $this->model_checkout_order->getOrders();
	...
	public function update(){
		...
		if (!isset($this->session->data['api_id'])) {
			...
			$shipDate = $this->request->post['shipDate'];
			$order_id = $this->request->post['order_id'];
			$query = "update `" . DB_PREFIX . "order` set tracking = '$tracking', order_status_id = 3, date_modified = '$shipDate' where order_id = $order_id";
	        ...
	        if ($id) {
	            ...
	            $success = false;
	            $output = '';
	            $message = 'There was an error in updating the order';
	        }
	        $json = array($success, $output, $message);
		 }
