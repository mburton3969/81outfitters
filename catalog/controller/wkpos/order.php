...
class ControllerWkposOrder extends Controller {
	public function index() {
        ...
        $weight_product = $this->model_wkpos_order->getproductWeight($product['product_id'], $order['order_id'],$product['order_product_id']);
				$data['products'][] = array(
	...
	public function addOrder() {
		...
		if ($this->request->post['payment_method'] == 'cash') {
			$order_data['payment_method'] = $this->config->get('wkpos_cash_title' . $this->config->get('config_language_id'));
			$order_status_id = $this->config->get('wkpos_cash_order_status_id');
			if (!$order_data['payment_method'])
			$order_data['payment_method'] = $this->language->get('text_cash_payment');
		} elseif ($this->request->post['payment_method'] == 'card') {
			$order_data['payment_method'] = $this->config->get('wkpos_card_title' . $this->config->get('config_language_id'));
			$order_status_id = $this->config->get('wkpos_card_order_status_id');
			if (!$order_data['payment_method'])
			$order_data['payment_method'] = $this->language->get('text_card_payment');
		}
		if (isset($this->request->post['partial_credit_payment']) && $this->request->post['partial_credit_payment'] != 'false') {
			$order_data['payment_method'] .= ' + ' . $this->config->get('wkpos_credit_title'.$this->config->get('config_language_id'));
		} else if (isset($this->request->post['only_credit_payment']) && $this->request->post['only_credit_payment'] != 'false') {
			$order_data['payment_method'] = $this->config->get('wkpos_credit_title'.$this->config->get('config_language_id'));
			$order_data['payment_code'] = 'wkpos_credit';
			if (!$order_data['payment_method'])
			$order_data['payment_method'] = $this->language->get('text_credit');
		}
		// POS Update Code
		if (isset($address)) {
			...
			$order_data['shipping_firstname'] = $this->config->get('wkpos_firstname');
			$order_data['shipping_lastname'] = $this->config->get('wkpos_lastname');
			$order_data['shipping_company'] = $this->config->get('wkpos_company');
			$order_data['shipping_address_1'] = $this->config->get('wkpos_address_1');
			$order_data['shipping_address_2'] = $this->config->get('wkpos_address_2');
			$order_data['shipping_city'] = $this->config->get('wkpos_city');
			$order_data['shipping_postcode'] = $this->config->get('wkpos_postcode');
			$order_data['shipping_zone'] = $this->model_wkpos_wkpos->getZoneById($this->config->get('wkpos_zone_id'));
			$order_data['shipping_zone_id'] = $this->config->get('wkpos_zone_id');
			$order_data['shipping_country'] = $this->model_wkpos_wkpos->getCountryById($this->config->get('wkpos_country_id'));
			$order_data['shipping_country_id'] = $this->config->get('wkpos_country_id');
			$order_data['shipping_address_format'] = '';
			$order_data['shipping_custom_field'] = array();
			$order_data['shipping_code'] = '';
			$order_data['shipping_method'] = '';
		...
		if (isset($this->request->post['shipping_code'])) {
			...
			$order_data['shipping_method'] = $this->config->get('wkpos_home_delivery_title');
		}
			...
			foreach ($this->request->post['cart'] as $product) {
				...
				if (isset($product['custom'])) {
					...
					if ($product_info['special']) {
						if($this->config->get('oc_pricelist_status') && $this->config->get('wkpos_price_list_satus') && $customer_info){
								$customer_data = array(
										'id' 					=> $customer_info['customer_id'],
										'group_id' 		=> $customer_info['customer_group_id'],
										'address_id' 	=> $customer_info['customer_group_id'],
								);
								$product_info['special'] = $this->Pricelist->getPrice(array('product_id' => $product['product_id'], 'price' => $product_info['special'], 'customer_data' => $customer_data));
						}
						$price = $product_info['special'];
					} else {
						if($this->config->get('oc_pricelist_status') && $this->config->get('wkpos_price_list_satus') && isset($customer_info) && $customer_info){
								$customer_data = array(
										'id' 					=> $customer_info['customer_id'],
										'group_id' 		=> $customer_info['customer_group_id'],
										'address_id' 	=> $customer_info['customer_group_id'],
								);
								$product_info['price'] = $this->Pricelist->getPrice(array('product_id' => $product['product_id'], 'price' => $product_info['price'], 'customer_data' => $customer_data));
						}
   ...
   if (!isset($product['custom'])) {
		   ...
		   $price = $price;
			$total_price = $price * $product['quantity'];
				...
				$order_data['products'][] = array(
					...
					'weight_unit' => isset($product['weight_unit']) ? $product['weight_unit'] :0,
					'tax'        => isset($product_info['price']) && isset($product_info['tax_class_id']) ? $this->tax->getTax($product_info['price'], $product_info['tax_class_id']) : 0,
		...
		if ($this->config->get('coupon_sort_order') >= $this->config->get('tax_sort_order')) {
			$coupon_sort_order = 3;
			$tax_sort_order = 4;
		} else {
			$tax_sort_order = 3;
		...
		if ($this->request->post['shipping_method']) {
			$shipping_charge = (float)$this->request->post['shipping_charge'];
			$order_data['totals'][1] = array(
				'code'  => 'shipping',
				'title' => $this->language->get('text_rate_home_delivery'),
				'value' => $shipping_charge,
				'sort_order' => 1
			);
		}

		$total += (float)$shipping_charge;
		...
		if ($this->config->get('wkpos_tax_status') && isset($this->request->post['tax']) && ($this->request->post['tax'] > 0)) {
			...
			$order_data['totals'][$tax_sort_order] = array(
				'code'  => 'tax',
				'title' => $this->language->get('text_tax'),
				'value' => $tax,
				'sort_order' => $tax_sort_order
			);
			$total += $tax;
		}
		...
		if (isset($this->request->post['coupon']) && isset($this->request->post['coupon']['discount']) && $this->request->post['coupon']['discount']) {
			...
			$order_data['totals'][$coupon_sort_order] = array(
				'code'  => 'coupon',
		...
		$credit = 0;
		$add_credit = 0;
		if ((isset($this->request->post['use_credit']) && ($this->request->post['use_credit']) && $this->request->post['partial_credit_payment']) && (isset($this->request->post['credit']) && $this->request->post['credit'] > 0)) {
			$credit = $this->request->post['credit'];
			if (isset($this->request->post['partial_credit_payment']) && $this->request->post['partial_credit_payment']) {
				$order_data['totals'][6] = array(
					'code'  => 'wkpos_credit',
					'title' => $this->config->get('wkpos_credit_title'.$this->config->get('config_language_id')),
					'value' => -$credit,
					'sort_order' => 6
				);
			} else if (isset($this->request->post['only_credit_payment']) && $this->request->post['only_credit_payment']) {
				$order_data['totals'][6] = array(
					'code'  => 'wkpos_credit',
					'title' => $this->config->get('wkpos_credit_title'.$this->config->get('config_language_id')),
					'value' => -$credit,
		...
		if (isset($this->request->post['add_credit']) && $this->request->post['add_credit'] > 0) {
			$add_credit = $this->request->post['add_credit'];
		}
	...
	public function applyCoupon() {
		...
		$this->load->language('wkpos/coupon');
		...
		if (empty($this->request->post['coupon'])) {
			...
			$json['success'] = str_replace('Success:', '', $this->language->get('text_success'));
