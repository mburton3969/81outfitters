...
class ModelWkposProducts extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_barcode'])) {
			$sub_query = "LEFT JOIN `" . DB_PREFIX . "wkpos_barcode` wb ON (p.product_id = wb.product_id)";
		}

		$sql = "SELECT p.product_id, p.price, p.image, p.model, p.quantity, p.status, pd.name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) " . $sub_query . " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

		if (!empty($data['filter_barcode'])) {
			$sql .= " AND wb.barcode LIKE '" . $this->db->escape($data['filter_barcode']) . "%'";
		}
	...
	public function getPosQuantityStatus($product_id) {
		$sql = "SELECT sum(quantity) as pos_quantity, status as pos_status FROM `" . DB_PREFIX . "wkpos_products` WHERE product_id = ".$product_id;
		$query = $this->db->query($sql)->row;
		return $query;
	}

	public function getPosBarcode($product_id){
		$sql = "SELECT barcode FROM `" . DB_PREFIX . "wkpos_barcode` WHERE product_id = ".$product_id;
		$query = $this->db->query($sql)->row;
		if(isset($query['barcode']) && $query['barcode']){
	...
	public function getOutletProductData($product_id, $outlet) {
		$sql = "SELECT quantity, status, unit_price_status FROM " . DB_PREFIX . "wkpos_products WHERE product_id = '" . $product_id . "' AND outlet_id = '" . $outlet . "'";
		...
		return array(
			...
			'unit_price_status'   => isset($query->row['unit_price_status']) ? $query->row['unit_price_status'] : 0
			);
	}
	...
	public function generateBarcode($product_id) {
		...
		$query = $this->db->query("SELECT sku, upc, ean FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$product_id . "")->row;

		if (isset($query[$this->config->get('wkpos_generate_barcode')]) && $query[$this->config->get('wkpos_generate_barcode')]) {
			$product = $query[$this->config->get('wkpos_generate_barcode')];
		}
		...
		$pos_product = $this->db->query("SELECT * FROM " . DB_PREFIX . "wkpos_barcode WHERE product_id = '" . (int)$product_id . "'")->row;
		...
		if (isset($pos_product['barcode_id'])) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_barcode SET barcode = '" . $product . "', product_id = '" . (int)$product_id . "'");
		}
		return $product;
	}
	...
	public function getBarcode($product_id) {
		$pos_product = $this->db->query("SELECT pd.name, wb.barcode,p.* FROM " . DB_PREFIX . "wkpos_barcode wb LEFT JOIN " . DB_PREFIX . "product_description pd ON (wb.product_id = pd.product_id) LEFT JOIN ".DB_PREFIX."product p ON(p.product_id = pd.product_id) WHERE pd.language_id = '" . $this->config->get('config_language_id') . "' AND wb.product_id = '" . $product_id . "'")->row;
		return $pos_product;
	}
	...
	public function changeUnitPriceStatus($product_id, $state, $outlet) {
	$pos_product = $this->db->query("SELECT * FROM " . DB_PREFIX . "wkpos_products WHERE product_id = '" . $product_id . "' AND outlet_id = '" . (int)$outlet . "'")->row;

	if (isset($pos_product['wkpos_products_id'])) {
		$this->db->query("UPDATE " . DB_PREFIX . "wkpos_products SET unit_price_status = '" . $state . "' WHERE wkpos_products_id = '" . $pos_product['wkpos_products_id'] . "' AND outlet_id = '" . (int)$outlet . "'");
	} else {
		$this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_products SET status = '0', unit_price_status = '" . $state . "', product_id = '" . $product_id . "', outlet_id = '" . (int)$outlet . "'");
	}
}
public function getPosUnitPriceStatus($product_id){
	$sql = "SELECT unit_price_status FROM `" . DB_PREFIX . "wkpos_products` WHERE product_id = ".$product_id;
	$query = $this->db->query($sql)->row;
	return $query;
}
}
