...
class ControllerToolExportImport extends Controller { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateUploadForm())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if ($this->model_tool_export_import->upload($file,$this->request->post['incremental'])==true) {
					...
					$this->response->redirect($this->url->link('tool/export_import', 'user_token=' . $this->session->data['user_token'], $this->ssl));
				}
				else {
	...
	public function download() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateDownloadForm()) {
			...
			$this->response->redirect( $this->url->link( 'tool/export_import', 'user_token='.$this->request->get['user_token'], $this->ssl) );
		}
	...
	protected function getForm() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('tool/export_import', 'user_token=' . $this->session->data['user_token'], $this->ssl)
		);

		$data['back'] = $this->url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['button_back'] = $this->language->get( 'button_back' );
		$data['import'] = $this->url->link('tool/export_import/upload', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['export'] = $this->url->link('tool/export_import/download', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['settings'] = $this->url->link('tool/export_import/settings', 'user_token=' . $this->session->data['user_token'], $this->ssl);
