...
class ControllerWkposProducts extends Controller {
	public function index() {
		...
		$data['product_total'] = $this->model_catalog_product->getTotalProducts();
		$data['number_inslot'] = $this->config->get('wkpos_barcode_slot') ? $this->config->get('wkpos_barcode_slot') : 5;
	...
	public function loadProducts() {
		...
		if (isset($this->request->post['filter_barcode']) && $this->request->post['filter_barcode']) {
			$filter_barcode = $this->request->post['filter_barcode'];
		} else {
			$filter_barcode = null;
		}
		...
		foreach ($results as $result) {
			...
			if ($outlet) {
				...
				$pos_unit_price_status = $outlet_data['unit_price_status'];
		  	$result['barcode'] = '';
			} else {
				$pos_quantity = $this->model_wkpos_products->getPosQuantityStatus($result['product_id'])['pos_quantity'];
				$pos_status = $this->model_wkpos_products->getPosQuantityStatus($result['product_id'])['pos_status'];;
				$pos_unit_price_result = $this->model_wkpos_products->getPosUnitPriceStatus($result['product_id']);
			if(isset($pos_unit_price_result['unit_price_status'])) {
			$pos_unit_price_status = $pos_unit_price_result['unit_price_status'];
		} else {
			$pos_unit_price_status =0;
		}

			}

			$barcode_data = $this->model_wkpos_products->getPosBarcode($result['product_id']);

			if ($barcode_data && is_file(str_replace('system/', 'wkpos/barcode/img/', DIR_SYSTEM) . $barcode_data . '.png')) {
				$barcode = HTTPS_CATALOG . 'wkpos/barcode/img/' . $barcode_data . '.png?lastimage='.uniqid();
			} else {
				$barcode = 0;
			}
			...
			$json['products'][] = array(
				...
				'pos_unit_price_status' => $pos_unit_price_status
			);
		}
	...
	public function generatePrint() {
		...
		if (isset($this->request->post['product_id']) && $this->request->post['product_id']) {
			...
			$data['name'] = "Name:" . $barcode['name'];
			$data['price'] = "Price:" . $this->currency->format($this->tax->calculate($barcode['price'], $barcode['tax_class_id'], $this->config->get('config_tax')), $this->config->get('config_currency'));
...
//.uniqid() remove then show error
	...
	public function changeUnitPriceProduct() {
		...
		if (!$json && isset($this->request->post['product_id']) && $this->request->post['product_id'] && isset($this->request->post['state']) && isset($this->request->post['outlet'])) {
			$this->load->model('wkpos/products');
			$this->model_wkpos_products->changeUnitPriceStatus($this->request->post['product_id'], $this->request->post['state'], $this->request->post['outlet']);
			$json['success'] = $this->language->get('text_success_unit_price_status');
		}
