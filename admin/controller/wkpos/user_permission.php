...
class ControllerWkposUserPermission extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('wkpos/user_permission', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('wkpos/user_permission', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('wkpos/user_permission/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('wkpos/user_permission/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['user_groups'][] = array(
				...
				'name'          => $result['name'],
				'edit'          => $this->url->link('wkpos/user_permission/edit', 'user_token=' . $this->session->data['user_token'] . '&user_group_id=' . $result['user_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('wkpos/user_permission', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->link('wkpos/user_permission', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('wkpos/user_group_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['user_group_id'])) {
			$data['action'] = $this->url->link('wkpos/user_permission/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('wkpos/user_permission/edit', 'user_token=' . $this->session->data['user_token'] . '&user_group_id=' . $this->request->get['user_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('wkpos/user_permission', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('wkpos/user_group_form', $data));
	}
